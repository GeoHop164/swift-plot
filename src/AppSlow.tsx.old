...
export default function ExcelGraphApp() {
  ...
  const [fullData, setFullData] = useState<any[]>([]); // <- raw complete file
  const [visibleData, setVisibleData] = useState<any[]>([]); // <- what's drawn
  ...
  const [rendering, setRendering] = useState<boolean>(false); // new

  const tempBuffer = useRef<any[]>([]);
  const flushing = useRef<boolean>(false);
  const progressiveDrawTimer = useRef<NodeJS.Timeout | null>(null);
  ...
  const startFlushing = () => {
    ...
    const flushInterval = setInterval(() => {
      if (tempBuffer.current.length === 0) {
        ...
        setFinishedLoading(true); // file is fully loaded
        return;
      }
  ...
  const handleCheckboxChange = (col: string) => {
    setSelectedColumns(prev => {
      ...
      if (progressiveDrawTimer.current) {
        clearInterval(progressiveDrawTimer.current);
      }
      ...
      let currentIdx = 0;
      progressiveDrawTimer.current = setInterval(() => {
        setVisibleData(prev => {
          const nextChunk = fullData.slice(currentIdx, currentIdx + 500).map((row, idx) => {
            const obj: { [key: string]: any } = { idx: currentIdx + idx };
            newSelection.forEach((header) => {
              obj[header] = row[header];
            });
            obj[xAxisColumn] = row[xAxisColumn];
            return obj;
          });
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-4 relative">
      ...
      <div className="w-3/4 p-4 relative">
                ...
                <LineChart data={visibleData}>
                  ...
                  {selectedColumns.map((col, idx) => (
                    <Line
                      ...
                      activeDot={{ r: 1 }}
                      strokeWidth={1}
                      strokeOpacity={(finishedLoading && !rendering) ? 1 : 0.5}
                    />
                  ))}
                </LineChart>
              </ResponsiveContainer>
        ...
        {/* Progress bar */}
        {(!finishedLoading || rendering) && (
          <div className="absolute bottom-4 left-4 right-4">
            <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden">
              <div
                className="h-full bg-blue-500 transition-all duration-300"
                style={{
                  width: finishedLoading
                    ? rendering
                      ? "30%"
                      : "100%"
